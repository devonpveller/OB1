...
async function ingestThought(content, metadata) {
  ...
  const resp = await httpPost(
    ...
    // Both halves: `exposure` the COLUMN (NOT NULL, no default, what the H3 policies
    // read) and the `metadata.exposure` mirror (what the CURRENTLY DEPLOYED
    // `thoughts_ops_plane` policy reads - U5 shipped `ob_corpus_on_ops_plane(metadata)`
    // and a POST without the key is refused 42501). 'ops' conforms to the plane U5's
    // backfill already put every pre-existing row of this corpus on; it widens nothing
    // relative to 08-30 and does not settle where this corpus belongs, which is an open
    // PLAN 1.1 decision for the operator.
    {
      content,
      embedding,
